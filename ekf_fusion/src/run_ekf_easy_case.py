...
def main():
    ...
    man_col = 'maneuver_state_x' if 'maneuver_state_x' in raw_df.columns else 'maneuver_state'
    mask_straight = raw_df[man_col].isin(['straight', 'gentle_curve'])
    is_straight = mask_straight.astype(int)
    groups = (is_straight != is_straight.shift()).cumsum()
    
    longest_duration = 0.0
    best_start = None
    
    for group_id, group_df in raw_df[is_straight == 1].groupby(groups):
        t_start = group_df['timestamp_s'].min()
        t_end = group_df['timestamp_s'].max()
    ...
    if best_start is None:
        print("No straight/gentle_curve window found!")
        return
        
    start_time = best_start
    duration_s = min(longest_duration, 40.0) # Cap at 40s
    print(f"Found easy-case window: start={start_time:.2f}s, duration={duration_s:.2f}s")
    
    window_df = raw_df[(raw_df['timestamp_s'] >= start_time) & (raw_df['timestamp_s'] <= start_time + duration_s)]
    print(f"Maneuver states in this window: {window_df[man_col].unique()}")
    ...
    out_path = os.path.join(base_path, "ekf_fusion", "output", "IDRFusedEstimate_S1_blackout_easy_test.csv")
    ...
    print(f"\nGNSS genuinely reacquired at timestamp {reacq_timestamp:.2f}s")
    ...
    mask_pre = (out_df['timestamp_s'] >= start_time - 30.0) & (out_df['timestamp_s'] < start_time)
    pre_df = out_df[mask_pre]
    zupt_pct = (pre_df['zupt_active'].sum() / len(pre_df)) * 100.0 if len(pre_df) > 0 else 0.0
    print(f"1. ZUPT Activity (30s before blackout): {zupt_pct:.2f}% of rows were stationary")
    ...
    post_df = df[(df['timestamp_s'] >= reacq_timestamp)]
